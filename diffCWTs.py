...
class ResponsivePlot:
    def __init__(self, master):
        ...
        self.spectrum_mode_origin = None
        self.spectrum_mode_Morpho = None
        self.spectrum_mode_Denoised2 = None
        self.spectrum_mode_sav = None
    ...
    def _setup_layout(self):
        #self.gs = GridSpec(5, 2, figure=self.figure, left=0.08, right=0.98, bottom=0.08, top=0.95, hspace=0.3, wspace=0.25)
        """
        self.gs = GridSpec(5, 2, figure=self.figure)
        self.axes = [self.figure.add_subplot(pos) for pos in [
            ...
            self.gs[4, 0], self.gs[4, 1]
        ]]
        """
        self.axes = [self.figure.add_subplot(5,2, i+1) for i in range(10)]
    ...
    def _create_canvas(self):
        ...
        widget.grid(row=1, column=0, sticky='nswe')
    ...
    #def _on_resize(self, event):
        ...
        self.figure.set_size_inches(new_width/dpi, new_height/dpi, forward=True)

        #self.figure.tight_layout()
        self.figure.subplots_adjust(left=0.07, right=0.97, top=0.93, bottom=0.07, hspace=0.4, wspace=0.2)
        """
        #self.canvas.draw()
    ...
    def update_plots(self, processor):
        plots_config = [
            ...
            (4, "Denoised data2", [processor.denoised_data2], {'labels': ['Denoised data2']}),
            (6, "Final sav", [processor.final_sav], {'labels': ['Final sav']}),
            (8, "Final MONO", [processor.final_sav_ave], {'color': ['#b7d07a'], 'labels': ['Final MONO']})
        ]
    ...
    def update_fft_plots(self, processor):
        ...
        sig_ydata = processor.ydata[start_idx:end_idx]
        sig_morphology_filt = processor.morphology_filt[start_idx:end_idx]
        sig_denoised_data2 = processor.denoised_data2[start_idx:end_idx]
        sig_final_sav = processor.final_sav[start_idx:end_idx]
        ...
        def plot_spectrum(ax, signal, fs, mode, title, color):
            ...
            if mode == 'FFT':
                ...
                cbar = self.figure.colorbar(im, ax=ax, orientation='vertical', label='Amp', pad=0.08)
                self.colorbars.append(cbar)

        plot_spectrum(self.axes[1], sig_ydata, sample_rate, self.spectrum_mode_origin.get(), "Origin Signal", '#a4aca7')
        plot_spectrum(self.axes[3], sig_morphology_filt, sample_rate, self.spectrum_mode_Morpho.get(), "Morphology Filter", '#a4aca7')
        plot_spectrum(self.axes[5], sig_denoised_data2, sample_rate, self.spectrum_mode_Denoised2.get(), "Denoised 2", '#a4aca7')
        plot_spectrum(self.axes[7], sig_final_sav, sample_rate, self.spectrum_mode_sav.get(), "Final sav", '#a4aca7')
        plot_spectrum(self.axes[9], sig_final_sav_ave, sample_rate, self.spectrum_mode_mono.get(), "Final MONO", '#a4aca7')
...
class AdaptiveGUI:
    def __init__(self):
        ...
        self.timer_id = None
        self.plot_area.spectrum_mode_origin = self.spectrum_mode_origin
        self.plot_area.spectrum_mode_Morpho = self.spectrum_mode_Morpho
        self.plot_area.spectrum_mode_Denoised2 = self.spectrum_mode_Denoised2
        self.plot_area.spectrum_mode_sav = self.spectrum_mode_sav
    ...
    def _create_widgets(self):
        ...
        main_plots_frame.rowconfigure(0, weight=0)
        main_plots_frame.rowconfigure(1, weight=1)
        main_plots_frame.columnconfigure(0, weight=1)
    ...
    def _create_controls(self, parent):
        ...
        for param, label_text, min_, max_, step in param_config:
            ...
            slider = ttk.Scale(frame, from_=min_, to=max_,
                              command=lambda v, p=param, e=entry: self._slider_update(p, e, v))
        ...
        self.spectrum_mode_origin = tk.StringVar(value='FFT')
        self.spectrum_mode_Morpho = tk.StringVar(value='FFT')
        self.spectrum_mode_Denoised2 = tk.StringVar(value='FFT')
        self.spectrum_mode_sav = tk.StringVar(value='FFT')
        self.spectrum_mode_mono = tk.StringVar(value='FFT')
        mode_options = ['FFT', 'STFT', 'CWT']

        frame_origin = ttk.Frame(spectrum_mode_frame)
        frame_origin.pack(fill='x', pady=2)
        ttk.Label(frame_origin, text="Origin Signal Mode:").pack(side='left', padx=4)
        ttk.OptionMenu(frame_origin, self.spectrum_mode_origin, 'FFT', *mode_options,
                       command=lambda _: self.plot_area.update_fft_plots(self.processor)).pack(side='left')
        
        frame_morpho = ttk.Frame(spectrum_mode_frame)
        frame_morpho.pack(fill='x', pady=2)
        ttk.Label(frame_morpho, text="Morphology Filter Mode:").pack(side='left', padx=4)
        ttk.OptionMenu(frame_morpho, self.spectrum_mode_Morpho, 'FFT', *mode_options,
                       command=lambda _: self.plot_area.update_fft_plots(self.processor)).pack(side='left')
        
        frame_Denoised2 = ttk.Frame(spectrum_mode_frame)
        frame_Denoised2.pack(fill='x', pady=2)
        ttk.Label(frame_Denoised2, text="Denoised Data2 Mode:").pack(side='left', padx=4)
        ttk.OptionMenu(frame_Denoised2, self.spectrum_mode_Denoised2, 'FFT', *mode_options,
                       command=lambda _: self.plot_area.update_fft_plots(self.processor)).pack(side='left')
        
        frame_sav = ttk.Frame(spectrum_mode_frame)
        frame_sav.pack(fill='x', pady=2)
        ttk.Label(frame_sav, text="Final sav Mode:").pack(side='left', padx=4)
        ttk.OptionMenu(frame_sav, self.spectrum_mode_sav, 'FFT', *mode_options,
                       command=lambda _: self.plot_area.update_fft_plots(self.processor)).pack(side='left')
        
        frame_mono = ttk.Frame(spectrum_mode_frame)
        frame_mono.pack(fill='x', pady=2)
        ttk.Label(frame_mono, text="Final MONO Mode:").pack(side='left', padx=4)
        ttk.OptionMenu(frame_mono, self.spectrum_mode_mono, 'FFT', *mode_options,
                       command=lambda _: self.plot_area.update_fft_plots(self.processor)).pack(side='left')
